...
# Générer une palette de couleurs pour les 5 groupes de densité
niveaux_couleur <- 5
colors <- colorRampPalette(brewer.pal(9, "YlOrRd"))(niveaux_couleur)
...
# Utiliser la palette générée pour le graphique
...
# Charger les shapefiles avec sf
shapefile_path <- "../Donnees/data_carto/commune_francemetro_2021.shp"
fond_carte <- st_read(shapefile_path)

# Générer la palette de couleurs bleues
palette <- colorRampPalette(brewer.pal(9, "Blues"))(100)

# Définir les limites pour la carte
xlim <- c(-5, 0)  # Remplacer par vos valeurs
ylim <- c(44, 48)  # Remplacer par vos valeurs

# Construire la carte avec ggplot
ggplot() +
  geom_sf(data = fond_carte, fill = "lightgray", color = "gray") +
  geom_tile(data = recurrence_data, aes(x = x, y = y, fill = recurrence)) +  # Utilisez geom_tile au lieu de geom_raster
  scale_fill_gradientn(colours = palette, name = "Récurrence") +
  labs(title = "Carte de récurrence de la densité de poissons",
       subtitle = "Fréquence des points au-dessus du 90e percentile de la densité",
       x = "Longitude",
