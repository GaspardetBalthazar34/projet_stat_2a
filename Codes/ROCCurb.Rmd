...
plot(perf,, colorize = TRUE)
abline(a = 0, b = 1, lty = 2, col = "gray")
text(x = 0.6, y = 0.2, labels = paste("AUC =", round(auc_value, 3)), cex = 1.2, col = "red")
```

```{r}
# Calcul de l'AUC
perf <- performance(pred, "auc")
auc_value <- perf@y.values[[1]]  # Extraire la valeur de l'AUC
...
It might be better to compute the F1-score

```{r}
library(caret)

# Supposons que validation_predictions sont vos prédictions binaires (0 ou 1)
# et que validation_labels sont les vraies étiquettes
conf_matrix <- confusionMatrix(data = as.factor(validation_prob), reference = as.factor(validation_labels))
