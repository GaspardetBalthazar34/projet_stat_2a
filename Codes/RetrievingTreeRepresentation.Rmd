...
gr_logistic <- xgb.plot.tree(model = delta_model$logistic_model,
                             trees = delta_model$logistic_model$best_iteration,  # Assure-toi que cet index est correct et que le modèle contient au moins 99 arbres
                             plot_width = 2400,
                             plot_height = 3600,
                             render = FALSE,
...
Saving last tree from positive model

```{r}
# Préparation du graphique pour l'arbre spécifique
gr_positive <- xgb.plot.tree(model = delta_model$positive_model,
                             trees = delta_model$positive_model$best_iteration,  # Assure-toi que cet index est correct et que le modèle contient au moins 99 arbres
                             plot_width = 2400,
                             plot_height = 3600,
                             render = FALSE,
                             feature_names = delta_model$features  # Exclure la colonne cible si elle est incluse
