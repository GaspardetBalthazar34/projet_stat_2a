...
```{r}
# Keeps track of the rmse to make a rmse graph // nrounds
rmse_tracker <- list()

# function to do callbacks
cb <- function(env) {
  iter <- env$iteration
  rmse_tracker[[iter + 1]] <- env$evaluation$log[[1]]
}

train_delta_model <- function(data, target, nrounds = 200, save_model = FALSE, model_name = "delta_model"){
  library(xgboost)
  library(dplyr)
...
predict_delta_model <- function(model, newdata) {
  ...
  if (length(positive_indices) > 0) {
    ...
    # Applying the exponential function to invert the logarithmic transformation
    positive_predictions <- expm1(positive_predictions_log)
...
out = data_merlan %>%
  ...
  mutate(substrat = as.numeric(substrat))
