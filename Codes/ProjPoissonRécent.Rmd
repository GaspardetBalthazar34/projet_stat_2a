...
requiredPackages = c("xgboost", "caret", "dplyr", "corrplot", "ggplot2", "caret", "Ckmeans.1d.dp", "DiagrammeR","dismo", "raster", "scales", "fastDummies","scales","ROCR","sf","RColorBrewer")
...
afficher_dens_et_norm_log <- function(nom_variable) {
  ...
  ggplot(data_merlan, aes(x = log1p(!!sym(nom_variable)))) +
    ...
    # Calcul de la moyenne et de la variance
    stat_function(fun = dnorm, 
                  args = list(mean = mean(log1p(data_merlan[[nom_variable]])), 
                              sd = sd(log1p(data_merlan[[nom_variable]]))), 
                  color = "blue", 
                  size = 1) +
    
    # Légendes
    labs(title = paste0("Density Plot of log(", nom_variable,"+1)"),
         x = paste("log ",nom_variable),
         y = "Density") +
...
print(afficher_dens_et_norm("LogDensity"))
```
